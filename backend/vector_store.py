...
embedding_file_storage = LocalFileStore("./embedding_cache/")
embeddings = CacheBackedEmbeddings.from_bytes_store(
    base_embeddings,
    embedding_file_storage,
...
qdrant_client = QdrantClient(url=os.environ["QDRANT_URL"], api_key= os.environ["QDRANT_API_KEY"], timeout= 120)
...
def get_vector_store(session_id: str) -> QdrantVectorStore:
    ...
    if not qdrant_client.collection_exists(collection_name):
        qdrant_client.create_collection(collection_name,vectors_config=VectorParams(size=embedding_dim,distance=Distance.COSINE))
...
def list_papers(session_id : str) -> list[str]:
    ...
    while True:
        points, offset = qdrant_client.scroll(collection_name,with_payload=True, limit=100, offset=offset)
...
def search(query: str, session_id : str, k: int = 4) -> list[Document]:
    return get_vector_store(session_id).similarity_search(query, k)
